...
const Tokechart: React.FC<Props> = () => {
  ...
  const renderCustomizedLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent, index ,name, value}) => {
    ...
    return (
      <>
      <text x={x} y={y} fill="white" letterSpacing={3} textAnchor={x > cx ? 'start' : 'end'} dominantBaseline="central">
        {`${name} `}
      </text>
      <text x={x} y={y+20} fill="#c1d9e2" fontSize={12} fontWeight={400} textAnchor={x > cx ? 'start' : 'end'} dominantBaseline="central" className="pietext">
        {`${value}.00 %`}
      </text>
      </>
    );
 ...
 // @ts-ignore
  ...
  const handleResize = (width:any) => {
    console.log(width)
    return <PieChart width={width} height={width*0.8}>
    <Pie  stroke="none"   data={data01} dataKey="value" nameKey="name" cx="50%" cy="50%" fill="fill" innerRadius={width*0.25- 40} outerRadius={width*0.25}  label={renderCustomizedLabel} />
  </PieChart>
  ...
  return (
    ...
    data-wow-delay="0.3s">
        <h1>tokenomics <br/>
15% total buy/sell tax</h1>
        <div className="mychart wow fadeInDown"
    data-wow-duration="0.6s"
    data-wow-delay="0.3s">
